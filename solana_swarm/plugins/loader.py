"""
Plugin Loader for Solana Swarm Intelligence
...
class PluginLoader:
    ...
    async def load_all_plugins(self) -> Dict[str, AgentPlugin]:
        ...
        search_paths = [
            ("plugins", "plugin.py"),
            ("solana_swarm/agents", "plugin.py")
        ]
        
        # Only load specific plugins for Solana
        allowed_plugins = {
            'price-monitor',
            'decision-maker',
            'portfolio-manager',
    ...
    async def load_plugin(
        ...
        try:
            ...
            if not module:
                try:
                    module_path = f"solana_swarm.plugins.{name}"
                    module = importlib.import_module(module_path)
                except ImportError:
                    # Try loading from agents directory
                    agent_path = os.path.join("solana_swarm", "agents", name, "plugin.py")
